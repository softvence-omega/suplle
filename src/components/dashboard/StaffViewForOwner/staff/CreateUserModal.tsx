...
const roleOptions = [
  "manager", "dine-in", "waiter", "takeaway", "chef", "cashier", "maintenance",
];

// Zod schema without password
...
// Simple input style with border
const inputClass =
...
const CreateUserModal = ({ ButtonText }: { ButtonText: string }) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    // Example transformation - adjust based on server requirements
    return time
      .replace(/\./g, ':')  // Replace dots with colons (9.00 AM → 9:00 AM)
  ...
  formData.append("data", JSON.stringify(payload));
    ...
    try {
      const res = await axios.post("https://suplle-server-v2-2.onrender.com/api/v1/staff/create-staff", formData,  {
    headers: {
      Authorization: token, 
    },
  })
...
console.log(res, "response in modal")
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="max-h-[75vh] overflow-y-auto px-1 sm:px-4">
        ...
        <div className="mt-6">
          ...
          <select id="status" {...register("status")} className={inputClass} defaultValue="Active">
