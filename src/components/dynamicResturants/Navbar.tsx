    ...
    <div className='w-full p-5'>
      <nav className="flex items-center justify-between w-full h-16 bg-white shadow-md px-4">
        ...
        <div className="flex items-center">
          <Link to="/" className="flex items-center">
            ...
            <div className='flex flex-col'>
              <p className="text-2xl font-normal text-gray-800 ml-2 text-[]">Urban Bistro</p>
        ...
        <div className="hidden md:flex space-x-4 items-center gap-5">
          {
            navData.map((item) => (
              <Link key={item.id} to={item.url} className="text-black text-lg px-3 py-1 rounded-xl hover:bg-primary hover:text-white">
                {item.label}
              </Link>
            ))
          }
          <Link to='/'><button className='bg-primary-dark text-white px-5 py-2 rounded-3xl'>Login/Signup</button></Link>
        </div>
      ...
      {menuOpen && (
        <div className="md:hidden bg-white shadow-md px-4 py-3 space-y-3">
          ...
          <Link to='/' onClick={() => setMenuOpen(false)}>
            <button className='w-full bg-primary-dark text-white px-5 py-2 rounded-3xl'>Login/Signup</button>
          </Link>
        </div>
      )}
    </div>
