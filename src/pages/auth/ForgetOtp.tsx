...
import { resendCode, verifyPasswordOtp } from "@/store/features/auth/authSlice";
...
const ForgetOtp = () => {
  ...
  const { error, resetEmail } = useAppSelector((state) => state.auth);
  ...
  const maskEmail = (email: string | null): string => {
    ...
    const hiddenStars1 = "*".repeat(Math.max(0, atIndex - 3));
    const hiddenStars2 = "*".repeat(Math.max(0, dotComIndex - atIndex - 3));
  ...
  const handleSubmitOtp = () => {
    ...
    if (otp.length === 4) {
      dispatch(verifyPasswordOtp({ email: resetEmail, otp }))
        .unwrap()
        .then(() => {
          navigate("/reset-global-password");
        })
        .catch((err) => {
  ...
  const handleResendCode = () => {
    if (resetEmail) {
      dispatch(resendCode(resetEmail))
        .unwrap()
        .then((msg) => toast.success(msg))
        .catch((err) => toast.error(err || "Failed to resend code"));
    }
  };
